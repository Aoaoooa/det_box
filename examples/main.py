...
def detect(onnx_path='projects/det_box/checkpoints/best.onnx',img=r'box1.jpg',show=True):
    '''
    检测目标，返回目标所在坐标如：
...
async def create_upload_file():
    contents =('projects/det_box/checkpoints/best.onnx')  # 接收浏览器上传的图片
    im1 = BytesIO(contents)  # 将数据流转换成二进制文件存在内存中

    # 返回结果
    return detect(onnx_path='/home/weih/repo/yolov5/runs/train/exp9/weights/best.onnx', img=im1, show=False)